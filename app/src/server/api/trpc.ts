...
/**
 * This helper creates or gets the socket server IO if it has been already initialized
 */
...
export const createTRPCContext = (opts: CreateNextContextOptions) => {
  const { res } = opts;
  const wss = createWebSocketServer(res as NextApiResponseWithSocket);
  return createInnerTRPCContext({ wss });
};
